...
const std::string &MultipartParser::GenBodyContent()
{
    ...
    std::vector<std::future<std::string>> chunk_futures;
    for (auto &pair : chunkFiles_)
    {
        ChunkFileInfo chunkFile = pair.second;
        std::future<std::string> content_futures = std::async(std::launch::async, [&chunkFile]() {
            std::ifstream ifile(chunkFile.fileName, std::ios::binary | std::ios::ate);    
            std::streamsize size = (chunkFile.endPos > ifile.tellg()) ? (std::streamsize)ifile.tellg() :
                                                        (std::streamsize)(chunkFile.endPos - chunkFile.startPos + 1);
            ifile.seekg(chunkFile.startPos, std::ios::beg);
            char *buff = new char[size];
            ifile.read(buff, size);
            ifile.close();
            std::string ret(buff, size);
        ...
        chunk_futures.push_back(std::move(content_futures));
    }
    ...
    for (size_t i = 0; i < chunkFiles_.size(); ++i)
    {
        std::string filename = std::to_string(chunkFiles_[i].second.startPos) + "_" +
                               std::to_string(chunkFiles_[i].second.endPos);
        std::string content_type = "application/octet-stream";
        std::string file_content = futures[i].get();
        body_content_ += "\r\n--";
        body_content_ += boundary_;
        body_content_ += "\r\nContent-Disposition: form-data; name=\"";
        body_content_ += chunkFiles_[i].first;
        body_content_ += "\"; filename=\"";
        body_content_ += filename;
        body_content_ += "\"\r\nContent-Type: ";
        body_content_ += content_type;
